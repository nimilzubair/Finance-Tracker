// components/InstallmentsOverview.tsx - WITH CURRENCY CONTEXT (FIXED)
'use client';
...
const InstallmentsOverview = ({ showTitle = false }) => {
  ...
  const getNextPaymentAmount = (installment: Installment): string => {
    const amountPerPeriod = toNumber(installment.amountpermonth);
    return formatCurrency(amountPerPeriod);
  };

  const getPeriodsRemainingText = (installment: Installment): string => {
    const periodsRemaining =
      installment.periods_remaining ||
      installment.installmentdurationinmonths - installment.payments_made;

    switch (installment.payment_frequency) {
      case 'weekly': return `${periodsRemaining} weeks remaining`;
      case 'bi-weekly': return `${periodsRemaining} bi-weekly payments remaining`;
      case 'quarterly': return `${periodsRemaining} quarters remaining`;
      case 'yearly': return `${periodsRemaining} years remaining`;
      case 'custom': return `${periodsRemaining} payment periods remaining`;
      default: return `${periodsRemaining} months remaining`;
    }
  };
              ...
              >
                ...
                <div className="mb-3">
                  <div className="flex justify-between text-sm text-gray-600 dark:text-gray-300 mb-2">
                    <span>Paid: {formatCurrency(toNumber(installment.total_paid))}</span>
                    <span>Total: {formatCurrency(toNumber(installment.totalamount))}</span>
                  </div>
                  ...
                  <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-lg px-3 py-2 mb-3">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-yellow-800 dark:text-yellow-200">
                        Advance paid: {formatCurrency(toNumber(installment.advanceamount))}
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 pt-2 border-t border-gray-100 dark:border-gray-700">
                  <div>
                    ...
                    <span className="font-medium text-blue-600 dark:text-blue-400">
                      {formatCurrency(toNumber(installment.remaining_amount))}
                    </span>
                  </div>
                  <div>Created: {formatDate(installment.createdat)}</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="text-center p-3 bg-gray-50 dark:bg-gray-900/30 rounded-lg">
              <div className="text-xl font-bold text-gray-600 dark:text-gray-400 break-words">
                {formatCurrency(
                  installments.reduce((sum, inst) => sum + toNumber(inst.remaining_amount), 0)
                )}
              </div>
