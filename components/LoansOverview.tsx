// components/LoansOverview.tsx - WITH CURRENCY CONTEXT
'use client';
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
      {showTitle && (
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200 px-3 py-1 rounded-full text-sm font-medium">
            Total: {formatCurrency(totalRemaining)}
          </span>
        </div>
      )}
      ...
      {loans.length === 0 ? (
        ...
        <div className="space-y-4">
          {loans.map((loan) => {
            const paid =
              typeof loan.amountpaid === 'string'
                ? parseFloat(loan.amountpaid)
                : loan.amountpaid;
            const total =
              typeof loan.totalamount === 'string'
